...
describe('RolesService', () => {
  ...
  describe('create', () => {
    it('should throw BadRequestException if max_prestamos is missing', async () => {
      await expect(
        service.create({ dias_prestamo: 14 } as any),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if dias_prestamo is missing', async () => {
      await expect(service.create({ max_prestamos: 5 } as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException on supabase error', async () => {
      ...
      await expect(
        service.create({ max_prestamos: 5, dias_prestamo: 14 }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('update', () => {
    it('should throw BadRequestException on supabase error', async () => {
      ...
      await expect(service.update(1, { max_prestamos: 10 })).rejects.toThrow(
        BadRequestException,
      );
    });

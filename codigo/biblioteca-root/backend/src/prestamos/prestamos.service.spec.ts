...
describe('PrestamosService', () => {
  ...
  describe('create', () => {
    it('should issue a loan correctly', async () => {
      const dto = {
        id_usuario: 1,
        id_ejemplar: 10,
        fecha_devolucion_esperada: '2026-06-19',
        estado: 'ACTIVO',
        id_usuario_sistema: 1,
      };
      mockSupabaseClient.single.mockResolvedValue({
        data: { id_prestamo: 1, ...dto },
        error: null,
      });
    ...
    it('should throw BadRequestException if insertion fails', async () => {
      mockSupabaseClient.single.mockResolvedValue({
        data: null,
        error: { message: 'No exemplars available' },
      });
      await expect(service.create({ id_usuario: 1 } as any)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('findOne', () => {
    it('should return a loan if found', async () => {
      mockSupabaseClient.single.mockResolvedValue({
        data: { id_prestamo: 1, estado: 'ACTIVO' },
        error: null,
      });
  ...
  describe('update', () => {
    it('should modify loan states (e.g., DEVUELTO)', async () => {
      mockSupabaseClient.single.mockResolvedValue({
        data: { id_prestamo: 1, estado: 'DEVUELTO' },
        error: null,
      });
    ...
    it('should throw BadRequestException on error', async () => {
      ...
      await expect(service.update(1, { estado: 'DEVUELTO' })).rejects.toThrow(
        BadRequestException,
      );
    });
  });

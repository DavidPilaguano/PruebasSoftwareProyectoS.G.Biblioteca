...
describe('UsuariosService', () => {
    ...
    describe('create', () => {
        it('should throw BadRequestException if id_rol is missing', async () => {
            await expect(service.create({ primer_nombre: 'John', primer_apellido: 'Doe' } as any)).rejects.toThrow(BadRequestException);
        });
        ...
        it('should throw BadRequestException on supabase error', async () => {
            ...
            await expect(service.create({ id_rol: 1, primer_nombre: 'John', primer_apellido: 'Doe' })).rejects.toThrow(BadRequestException);
        });

        it('should create and return usuario data', async () => {
            const newUsuario = { id_usuario: 1, id_rol: 1, primer_nombre: 'John', primer_apellido: 'Doe', estado: 'ACTIVO' };
            mockSupabaseClient.single.mockResolvedValueOnce({ data: newUsuario, error: null });
            const result = await service.create({ id_rol: 1, primer_nombre: 'John', primer_apellido: 'Doe' });
        ...
        it('should use default estado ACTIVO when not provided', async () => {
            const newUsuario = { id_usuario: 1, id_rol: 1, primer_nombre: 'John', primer_apellido: 'Doe', estado: 'ACTIVO' };
            mockSupabaseClient.single.mockResolvedValueOnce({ data: newUsuario, error: null });
            await service.create({ id_rol: 1, primer_nombre: 'John', primer_apellido: 'Doe' });
    ...
    describe('findAll', () => {
        ...
        it('should return array of usuarios', async () => {
            const usuarios = [{ id_usuario: 1, primer_nombre: 'John' }, { id_usuario: 2, primer_nombre: 'Jane' }];
            mockSupabaseClient.select.mockResolvedValueOnce({ data: usuarios, error: null });
    ...
    describe('findOne', () => {
        ...
        it('should return usuario data', async () => {
            mockSupabaseClient.single.mockResolvedValueOnce({ data: { id_usuario: 1, email: 'test@test.com' }, error: null });
    ...
    describe('update', () => {
        it('should throw BadRequestException on supabase error', async () => {
            ...
            await expect(service.update(1, { primer_nombre: 'Jane' })).rejects.toThrow(BadRequestException);
        });

        it('should update and return usuario data', async () => {
            const updatedUsuario = { id_usuario: 1, primer_nombre: 'Jane', primer_apellido: 'Doe' };
            mockSupabaseClient.single.mockResolvedValueOnce({ data: updatedUsuario, error: null });

...
describe('AutoresService', () => {
  ...
  describe('create', () => {
    it('should create an author successfully', async () => {
      ...
      mockSupabaseClient.single.mockResolvedValueOnce({ data: { id_autor: 1, ...dto }, error: null });
    ...
    it('should throw BadRequestException if creation fails', async () => {
      mockSupabaseClient.single.mockResolvedValueOnce({ data: null, error: { message: 'Error DB' } });
      await expect(service.create({ primer_nombre: 'A', primer_apellido: 'B' } as any)).rejects.toThrow(BadRequestException);
    });
  });

  describe('findOne', () => {
    it('should return author if found', async () => {
      mockSupabaseClient.single.mockResolvedValueOnce({ data: { id_autor: 1 }, error: null });
  ...
  describe('update', () => {
    it('should update author correctly', async () => {
      mockSupabaseClient.single.mockResolvedValueOnce({ data: { id_autor: 1, primer_nombre: 'Nuevo' }, error: null });
    ...
    it('should throw NotFoundException if author does not exist', async () => {
      ...
      mockSupabaseClient.single.mockResolvedValueOnce({ data: null, error: null });
      await expect(service.update(1, { primer_nombre: 'Test' } as any)).rejects.toThrow(NotFoundException);
    });
  });
